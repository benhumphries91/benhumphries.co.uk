...
function submitForm(form,fields)
{
	...
	if ($('[name=email]').val().length === 0) {
		...
		formError('[name=email]', 'Please Enter an E-mail Address');
		error = error + 1;
	}
	if ($('[name=fullname]').val().length === 0) {

		formError('[name=fullname]', 'Please Enter Your First Name');
		error = error + 1;
	}
	if ($('[name=tel]').val().length === 0) {

		formError('[name=tel]', 'Please Enter a Contact Number');
		error = error + 1;
	}
	if ($('[name=message]').val().length === 0) {

		formError('[name=message]', 'Please Enter a Question');
		error = error + 1;
	}
	...
	if (error > 0) {
		...
		// need a for loop to build up the fields array
		...
		$.ajax({
			...
			success: function(ajax) {
				...
				if (ajax.error != null) {
					...
					// Database error 
					if (ajax.error['db']) {
						formError('.form-area', ajax.error['db'], 'db');
					} else {
						// user input error
				...
				if (ajax.success != null)
				{
					...
					$('.form-area').hide();
					$('.question-panel .title-top').text(ajax.success.title);
					$('.success').show().append('<p>'+ ajax.success.message +'</p><p>'+ ajax.success.response +'</p>');
